...
# Navigate to the correct group (matches the folder structure under VultisigApp/)
...
# Example: add files under VultisigApp/Services/MyFeature/
group = find_or_create_group(project, ['VultisigApp', 'Services', 'MyFeature'])

['MyService.swift', 'MyModel.swift'].each do |filename|
...
**Important notes:**
- The `path_components` array must match the group hierarchy in the Xcode project (starting from `VultisigApp`)
- Files in the same Xcode build phase must have **unique filenames** across the entire project (e.g., don't have two `AppDelegate.swift` files — use `MacAppDelegate.swift` for macOS)
- Always run `project.save` after modifications
- After adding files, verify with a build: `xcodebuild ... build`
