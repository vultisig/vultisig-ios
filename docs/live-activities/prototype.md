...
Every nonterminal content state has a stale date 90 seconds after its last actual transaction observation. A healthy RPC lookup returning notFound supplies no newer transaction evidence, so the last positive observation timestamp remains unchanged. During the chain’s first polling interval after broadcast, a missing hash emits no delay; later misses show an update delay. Foreground and native background observations use the same grace rule. Network errors, missing provider indexing and expiration never become transaction failure. Ordinary foreground tracking resumes when the app returns.
