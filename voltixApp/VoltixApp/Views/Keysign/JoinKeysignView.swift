...
struct JoinKeysignView: View {
                ...
                case .FailedToStart:
                    VStack{
                        Text(NSLocalizedString("keysignFail", comment: "Failed to start the keysign process"))
                            .font(.body15MenloBold)
                            .multilineTextAlignment(.center)
                        Text(errorMsg).font(.body15MenloBold)
                            .multilineTextAlignment(.center)
                    }
                }
            }
    ...
    var waitingForKeySignStart: some View {
        VStack {
            ...
            HStack {
                Text(NSLocalizedString("waitingForKeysignToStart", comment: "Waiting for the keysign process to start"))
                    .font(.body15MenloBold)
                    .foregroundColor(.neutral0)
                    .multilineTextAlignment(.center)
                ProgressView()
    ...
    private func prepareKeysignMessages(keysignPayload: KeysignPayload) {
        guard let vault = appState.currentVault else {
            self.logger.error("current vault is nil")
            self.currentStatus = .FailedToStart
            return
        }
