...
private extension SwapCryptoViewModel {
    ...
    func amount(for coin: Coin, tx: SwapTransaction) -> Int64 {
        ...
        case .ethereum, .avalanche,.arbitrum, .bscChain, .base, .optimism, .polygon, .blast, .cronosChain, .pulseChain:
    ...
    func swapAsset(for coin: Coin) -> THORChainSwapAsset {
        return THORChainSwapAsset.with {
            ...
            case .gaiaChain:
                $0.chain = .atom
            case .solana, .dash, .kujira, .mayaChain, .arbitrum, .base, .optimism, .polygon, .blast, .cronosChain, .pulseChain: break
            }
