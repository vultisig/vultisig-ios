...
enum EthereumFunction {
    static func transferErc20Encoder(address: String, amount: BigInt) throws -> String {
        ...
        guard address.isNotEmpty else { throw HelperError.runtimeError("Address cannot be blank") }
        
        let destinationAddress = AnyAddress(string: address, coin: CoinType.ethereum)
