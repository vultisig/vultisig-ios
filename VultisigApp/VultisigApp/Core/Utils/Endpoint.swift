...
class Endpoint {
    ...
    static func fetchKyberSwapRoute(chain: String, tokenIn: String, tokenOut: String, amountIn: String, saveGas: Bool, gasInclude: Bool, slippageTolerance: Int, affiliateBps: Int, sourceIdentifier: String? = nil, referrerAddress: String? = nil) -> URL {
        ...
        let affiliateParams = affiliateBps > 0 && sourceIdentifier != nil && referrerAddress != nil
            ? "&source=\(sourceIdentifier!)&referral=\(referrerAddress!)&feeAmount=\(affiliateBps)&chargeFeeBy=currency_out&isInBps=true&feeReceiver=\(referrerAddress!)"
            : .empty
