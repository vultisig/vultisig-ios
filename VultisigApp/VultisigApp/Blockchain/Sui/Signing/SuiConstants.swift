...
enum SuiCoinType {
    ...
    /// Validates a Sui Move struct tag, including nested generic type parameters.
    /// Short package addresses are accepted and normalized by Sui nodes, while
    /// module and type names follow Move identifier rules.
    static func isValidStructTag(_ value: String) -> Bool {
        var parser = SuiStructTagParser(value.trimmingCharacters(in: .whitespacesAndNewlines))
        return parser.parse()
    }
...
private struct SuiStructTagParser {
    ...
    mutating func parse() -> Bool {
        guard parseStructTag() else { return false }
        return isAtEnd
    }

    private mutating func parseStructTag() -> Bool {
        guard parsePackageAddress(), consume("::"), parseIdentifier(), consume("::"), parseIdentifier() else {
            return false
        }

        if consume("<") {
            skipWhitespace()
            guard parseTypeTag() else { return false }
            skipWhitespace()
            while consume(",") {
                skipWhitespace()
                guard parseTypeTag() else { return false }
    ...
    private mutating func parseTypeTag() -> Bool {
        ...
        let start = index
        if let identifier = readIdentifier(), Self.primitiveTypes.contains(identifier) {
            return true
        }
        index = start
    ...
    private mutating func parsePackageAddress() -> Bool {
        let start = index
        _ = consume("0x") || consume("0X")
        let hexStart = index

        while !isAtEnd, characters[index].isASCII, characters[index].isHexDigit {
            index += 1
        }

        let hexLength = index - hexStart
    ...
    private mutating func readIdentifier() -> String? {
        guard !isAtEnd, characters[index].isASCII, characters[index].isLetter else {
            return nil
        }
    ...
    private mutating func consume(_ value: String) -> Bool {
        ...
        guard index + expected.count <= characters.count,
              Array(characters[index..<(index + expected.count)]) == expected else {
            return false
        }
