...
struct RippleAccountResponse: Codable {
    ...
    struct Result: Codable {
        ...
        let status: String?
        let validated: Bool?
        /// Node-level JSON-RPC error string returned in an HTTP-200 body, e.g.
        /// `actNotFound` for accounts that don't exist on-ledger — a valid
        /// "unfunded account" outcome, intentionally not retryable — or a
        /// transient node error like `amendmentBlocked` that IS retryable.
        /// Previously undecoded — such responses decoded as all-nil.
        let error: String?
