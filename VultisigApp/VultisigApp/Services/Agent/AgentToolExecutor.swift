...
final class AgentToolExecutor {
    ...
    private static func executeListVaults(action: AgentBackendAction) async -> AgentActionResult {
    ...
    private static func executeGetAddresses(action: AgentBackendAction, vault: Vault) async -> AgentActionResult {
    ...
    private static func executeSearchToken(action: AgentBackendAction, vault: Vault) async -> AgentActionResult {
    ...
    private static func executeRemoveToken(action: AgentBackendAction, vault: Vault) async -> AgentActionResult {
        guard let paramsDict = action.params,
              let paramsData = try? JSONEncoder().encode(paramsDict) else {
            return buildErrorResult(action: action, error: "invalid_params")
        }
    ...
    private static func executeRemoveChain(action: AgentBackendAction, vault: Vault) async -> AgentActionResult {
        guard let paramsDict = action.params,
              let paramsData = try? JSONEncoder().encode(paramsDict) else {
            return buildErrorResult(action: action, error: "invalid_params")
        }
