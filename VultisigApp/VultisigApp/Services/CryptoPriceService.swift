...
private extension CryptoPriceService {
    ...
    func fetchPrices(contracts: [String], chain: Chain, coins: [CoinMeta] = []) async throws {
        ...
        } else if chain == .thorChain || chain == .thorChainChainnet || chain == .thorChainStagenet2 {
    ...
    private func coinGeckoPlatform(chain: Chain) -> String {
        ...
        case .mantle:
            return "mantle"
        case .thorChain, .thorChainChainnet, .thorChainStagenet2, .solana, .bitcoin, .bitcoinCash, .litecoin, .dogecoin, .dash, .gaiaChain, .kujira, .mayaChain, .cronosChain, .polkadot, .dydx, .sui, .ton, .osmosis, .terra, .terraClassic, .noble, .ripple, .akash, .tron, .zcash, .cardano, .hyperliquid, .sei:
            return .empty
        }
    }
}
