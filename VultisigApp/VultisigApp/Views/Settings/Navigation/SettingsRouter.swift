...
struct SettingsRouter {
    ...
    func build(_ route: SettingsRoute) -> some View {
        ...
        case .referralOnboarding(let referredViewModel):
            viewBuilder.buildReferralOnboardingScreen(referredViewModel: referredViewModel.object)

        case .referrals:
            viewBuilder.buildReferralsScreen()
        }
    }
}
