...
struct VaultSetupScreen: View {
    ...
    var body: some View {
        ...
        .crossPlatformToolbar {
            if currentStep > 0 {
                CustomToolbarItem(placement: .leading, hideSharedBackground: true) {
                    ToolbarButton(image: "chevron-right", action: navigateBack) { icon in
                        icon.rotationEffect(.degrees(180))
                    }
                }
            }
