...
struct YieldVaultScreen: View {
    ...
    /// Windowed-vault "Next redemption" value. VULT reads each request's own
    /// on-chain maturity and shows the soonest; a provider with a computed global
    /// settlement window has no source wired up, so it shows "--".
    private var nextRedemptionText: String {
        guard !presentation.usesComputedSettlementWindow else { return "--" }
    ...
    /// provider's deposited asset (USDC for Circle, VULT for staking).
