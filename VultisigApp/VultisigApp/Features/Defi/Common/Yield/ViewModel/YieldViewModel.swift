...
/// `DefiYieldProvider`. Replaces the Circle-specific view model so every provider
/// renders through one shell.
@MainActor
final class YieldViewModel: ObservableObject, Hashable, Equatable {
    ...
    /// Providers without an account-setup step (direct EOA) are always
