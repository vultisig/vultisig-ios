...
final class YieldViewModel: ObservableObject, Hashable, Equatable {
    ...
    /// requests; Circle/Noon have ≤1, so this is a 0-or-1 list for them.
    var claimableRedemptions: [YieldRedemption] {
        redemptions.filter { $0.status == .claimable && $0.isClaimable }
    }

    /// All pending (not-yet-claimable) redemptions. ≤1 for Circle/Noon.
    var pendingRedemptions: [YieldRedemption] {
        redemptions.filter { $0.status == .pending }
    }

    /// Single-redemption shims for any non-list call site (Circle/Noon keep ≤1).
