...
struct SwapCryptoLogic {
    ...
    func baseAffiliateFee(tx: SwapTransaction) -> String {
        ...
        case let .kyberswap(q, _):
            let affiliateBps = max(0, 50 - tx.vultDiscountBps)
            guard affiliateBps > 0, let dstAmountRaw = Decimal(string: q.dstAmount) else {
                return .empty
            }
            let decimals = tx.toCoin.decimals
            let dstAmountDecimal = dstAmountRaw / pow(10, decimals)
            let feeDecimal = dstAmountDecimal * Decimal(affiliateBps) / 10000
