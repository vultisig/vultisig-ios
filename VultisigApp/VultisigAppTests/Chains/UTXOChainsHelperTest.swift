...
final class UTXOChainsHelperTest: XCTestCase {
    ...
    func testGetCoin() throws {
        ...
        let bch = try CoinFactory.create(asset: TokensStore.Token.bitcoinCash, vault: vaultForTest)
        XCTAssertNotNil(bch)
        ...
        let ltc = try CoinFactory.create(asset: TokensStore.Token.litecoin, vault: vaultForTest)
        ...
        let doge = try CoinFactory.create(asset: TokensStore.Token.dogecoin, vault: vaultForTest)
    ...
    func testGetPreSignedImageHash() throws {
        ...
        vaultForTest.hexChainCode = hexChainCode
        let btc = try CoinFactory.create(asset: TokensStore.Token.bitcoin, vault: vaultForTest)
        let result = try utxoHelper.getPreSignedImageHash(keysignPayload: KeysignPayload(coin: btc,
    ...
    func testGetBitcoinCashPreSignedImageHash() throws {
        ...
        vaultForTest.hexChainCode = hexChainCode
        let bch = try CoinFactory.create(asset: TokensStore.Token.bitcoinCash, vault: vaultForTest)
        let result = try utxoHelper.getPreSignedImageHash(keysignPayload: KeysignPayload(coin:bch,
        ...
        let signedTxResult = try utxoHelper.getSignedTransaction(keysignPayload: KeysignPayload(coin: bch,
                                                                                                toAddress: "bitcoincash:qqxjcn4u4fgxvclqyaprkem3hptm3nf5yq3ryq70ry",
                                                                                                toAmount: 1000000,
                                                                                                chainSpecific: BlockChainSpecific.UTXO(byteFee: 20,sendMaxAmount: false),
                                                                                                utxos: [UtxoInfo(hash: "71787a90556de944fcea8d8ff7478e535092638a68491b60b5661dfd871c40e4", amount: 10000000, index: 0)],
