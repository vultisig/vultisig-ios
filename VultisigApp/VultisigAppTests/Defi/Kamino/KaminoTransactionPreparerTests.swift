...
final class KaminoTransactionPreparerTests: XCTestCase {
    ...
    func testTheWithdrawRequestIsDenominatedInShares() async throws {
        ...
        XCTAssertEqual(harness.service.withdrawRequests.first?.vault, KaminoVaultRegistry.steakhouseUSDC.address)
...
private final class StubKaminoService: KaminoServiceProtocol, @unchecked Sendable {
    ...
    private var _withdrawRequests: [(owner: String, vault: String, shares: KaminoShareAmount)] = []

    var depositRequests: [(owner: String, vault: String, amount: KaminoTokenAmount)] {
        lock.withLock { _depositRequests }
    }

    var withdrawRequests: [(owner: String, vault: String, shares: KaminoShareAmount)] {
