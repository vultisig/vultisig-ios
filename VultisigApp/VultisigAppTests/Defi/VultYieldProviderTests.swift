...
//  Circle/Noon against regression from the new optional protocol method.
//
...
final class VultYieldProviderTests: XCTestCase {
    ...
    // MARK: - Cancel defaults (Circle/Noon regression guard)

    func testCircleDoesNotSupportCancel() {
        XCTAssertFalse(CircleYieldProvider().presentation.supportsCancel)
    }

    func testNoonDoesNotSupportCancel() {
        XCTAssertFalse(NoonYieldProvider().presentation.supportsCancel)
    }

    func testNoonStillUsesComputedSettlementWindow() {
        // The presentation default keeps Noon's weekly-window behaviour intact.
        XCTAssertTrue(NoonYieldProvider().presentation.usesComputedSettlementWindow)
        XCTAssertEqual(NoonYieldProvider().presentation.assetTicker, "USDC")
    }
