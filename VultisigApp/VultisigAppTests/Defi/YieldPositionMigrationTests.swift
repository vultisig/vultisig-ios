...
final class YieldPositionMigrationTests: XCTestCase {
    ...
    func testUpsertWritesRedemptionRows() throws {
        let vault = TestStore.makeVault(pubKey: "noon-rows")
        ...
        let position = try XCTUnwrap(service.position(for: vault, providerID: .noon))
    ...
    func testUpsertReplacesStaleRedemptionRows() throws {
        let vault = TestStore.makeVault(pubKey: "noon-replace")
        let pending = YieldRedemption(id: "req-1", amount: 95, requestedAt: .now, claimableAt: .now, status: .pending)
        try service.upsert(providerID: .noon, depositedBalance: 100, nativeGasBalance: 0.2, redemptions: [pending], for: vault)

        let claimable = YieldRedemption(id: "req-1", amount: 95, requestedAt: .now, claimableAt: .now, status: .claimable)
        try service.upsert(providerID: .noon, depositedBalance: 100, nativeGasBalance: 0.2, redemptions: [claimable], for: vault)
