//
//  SwitchAmountTests.swift
//  VultisigAppTests
//
//  The human-decimal parse the SWITCH amount is read through. SWITCH attaches a
//  real transfer of the chain's native asset, so an amount reinterpreted under
//  the wrong separator convention is a ten-times send — the defect the UNMERGE
//  migration's review caught in the same layer.
//

@testable import VultisigApp
import XCTest

final class SwitchAmountTests: XCTestCase {
    ...
    private func parse(_ text: String, decimals: Int = SwitchAmountTests.atomDecimals, locale: Locale) -> Decimal? {
        SwitchAmount.parse(text, decimals: decimals, locale: locale)
    }
    ...
    /// ten-times transfer from a paste, so both are refused and the form says
    ...
    func testAnAmountInTheLocalesOwnDigitsIsAccepted() throws {
        ...
        XCTAssertEqual(SwitchAmount.parse(text, decimals: Self.atomDecimals, locale: egypt), original)
    }
    ...
    func testTheFormattersOutputParsesBackToTheSameValue() {
        for value in ["1", "1.5", "12345.678901", "0.000001"] {
            ...
            XCTAssertEqual(
                SwitchAmount.parse(formatted, decimals: Self.atomDecimals, locale: .current),
                original,
