...
- Add every new key to every locale listed in `VultisigApp/scripts/sort_localizable.py`'s `LOCALE_DIRS`, including `ko.lproj`; that list is the source of truth
...
The authoritative locale set is `LOCALE_DIRS` in `VultisigApp/scripts/sort_localizable.py`, which currently enumerates eight locales and includes `ko.lproj`. Each entry maps to `VultisigApp/VultisigApp/Core/Localizables/<locale>/Localizable.strings`.
