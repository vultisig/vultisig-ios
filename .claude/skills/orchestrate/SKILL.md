...
- Agent C: Localization (every locale in `sort_localizable.py`'s `LOCALE_DIRS`, including `ko.lproj`)
- Agent D: Tests
...
- **Localization Agent**: Every locale in `LOCALE_DIRS` updated, including `ko.lproj`; camelCase keys, alphabetical order
...
- [ ] Localization: every locale in `LOCALE_DIRS`, including `ko.lproj`, updated and sorted
